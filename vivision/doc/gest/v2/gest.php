...
function genbuttons(id){
	var h='<td><input type="button" id="edit_button1" value="Edit" class="edit" onclick="edit_row('+id+')">';
	h+='<input type="button" id="save_button1" value="Save" class="save" onclick="save_row('+id+')">';
	h+='<input type="button" value="Delete" class="delete" onclick="delete_row('+id+')">';
	h+='</td>';
	return h;
}

getdata('select * from tabFund');
function getdata(sql){
	...
	xhttp.onreadystatechange = function () { if (this.readyState == 4 && this.status == 200) {
		...
		var head = JSON.parse(xhttp.responseText); 
		console.log('head',head);
		for(var i=0;i<head.length;i++)
		console.log('head',head[i]['name']);
		var send=xhttp.responseText;
		
		var html="";
		html+="<table align=´center´ cellspacing=2 cellpadding=5 id="+data_table+" border=1><tr>";
		for(var i=0;i<head.length;i++)
		...
		xhttp.onreadystatechange = function () { if (this.readyState == 4 && this.status == 200) {
			...
			html="";
			// console.log('obj',xhttp.responseText);
			console.log('obj',obj);
			for(var i=0;i<obj.length;i++){
			
				html='<tr id="row'+(i+1)+'">'; //MUDAR AQUI PARA MULTIPLE TABLES
				for(var j=0;j<head.length;j++){
					html+="<td>";
					html+= obj[i][ head[j]['name'] ];
					html+="</td>";
					console.log(html);
				}
				//addbuttons
...
function genhead(data_table,jsn){
	var head="<table align=´center´ cellspacing=2 cellpadding=5 id="+data_table+" border=1><tr>";
	head+="<th>Name</th><th>Country</th>	<th>Age</th>	</tr>";
	head+="</tr>";
	return head;
}
function gen(){
