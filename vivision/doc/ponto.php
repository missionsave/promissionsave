...
// $startUTC= ((new DateTime($start))->setTimezone(new DateTimeZone('UTC')));	
	$faseidsel=@$_POST['faseidsel'];
	date_default_timezone_set('UTC');
	if($write==1 && $faseidsel!='') //é login
		if($btfunc=="Stop" && $_POST['btfunc']=="Stop" ){
			$stmt3 = $db -> prepare("UPDATE  tabPontos SET idfase=? ,end=?,obs=? WHERE id=?;");
			$stmt3 -> execute(array($faseidsel, (date("Y-m-d H:i:s", time())), $_POST['obs'], $id));
				$stmt -> execute();
				$res = $stmt->fetchAll(PDO::FETCH_ASSOC);
				$btfunc="Start";
			...
			$stmt3 = $db -> prepare("INSERT INTO tabPontos(idWorker, idfase, start) VALUES ( ? , ? , ? )");
			$stmt3 -> execute(array($idWorker,$faseidsel,(date("Y-m-d H:i:s", time()))));	
			// $startUTC=strtotime((gmdate("Y-m-d H:i:s", time())));
				$btfunc="Stop";
		}else {
...
if($btfunc=="Stop"){
	...
	echo " Started at ".(new DateTime($start))->setTimezone(new DateTimeZone('Europe/Lisbon'))->format("Y-m-d H:i:s")." Elapsed ".'<span id="datetime"></span>';
}
...
echo '<input onclick="" style="color:yellow; background-color: '.$btc.';" name="'.'btfunc'.'" type="submit" value="'.$btfunc.'">';
...
// $start=(new DateTime($start))->setTimezone(new DateTimeZone('UTC'));
// $start=$start->format('Y-m-d H:i:s');
// echo "d". gmmktime($start)." ".(new DateTime( ($start)))->setTimezone(new DateTimeZone('UTC'))->format('Y-m-d H:i:s');
?>
</table> 
<script>
function timef(s) {
    return  new Date(s  ).toISOString().slice(-13, -5);
    // return  new Date(<?php echo strtotime($start) ?>*1000);
}
var myVar = setInterval(myTimer1, 1000);
var dt1 = new Date(<?php echo strtotime($start) ?>*1000); 
// var d = new Date("<?php echo $start; ?>").getTime();
// var dt = new Date(<?php echo $startUTC; ?>);
var dt = new Date();
// var dt = new Date(Date.UTC("<?php echo $start; ?>"));
var d=dt1.getTime();
// var dt1=new Date();
console.log("ds","<?php echo $startUTC; ?>");
console.log("d",dt.toISOString());

// console.log("d",(dt1.getTime()).toISOString());
function myTimer1() {
  if(typeof datetime == "undefined")return;
  var time = new Date().getTime() - d;  
  datetime.innerHTML = timef(time );
}
</script>
