	...
	while ((ch = getch()) != 'q') {
		...
		if (ch == 'r') 
		{
			for (i = 0; i != GAMEPAD_COUNT; ++i) {
				...
				GamepadSetRumble((GAMEPAD_DEVICE)i, 0.1, 0);
				sleep(1);
				GamepadSetRumble((GAMEPAD_DEVICE)i, 1, 0.1);
				sleep(1);
				GamepadSetRumble((GAMEPAD_DEVICE)i, 0.1, 1);
				sleep(1);
				GamepadSetRumble((GAMEPAD_DEVICE)i, 1, 1);
				sleep(1);
				GamepadSetRumble((GAMEPAD_DEVICE)i, 0, 0);
